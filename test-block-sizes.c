...
extern void square_dgemm (int, double*, double*, double*, int, int);
...
//    for (int nbl = 0; nbl < sizeof(block_sizes); nbl++)
    {
      int M_BLOCK_SIZE = m_block_sizes[bl];
      int N_BLOCK_SIZE = n_block_sizes[bl];
      ...
      for (int isize = 0; isize < sizeof(test_sizes)/sizeof(test_sizes[0]); ++isize)
      {
        ...
        /* perform standard benchmarking code */
        for (int n_iterations = 1; seconds < timeout; n_iterations *= 2) 
        {
          /* Warm-up */
          square_dgemm (n, A, B, C, M_BLOCK_SIZE, N_BLOCK_SIZE);
...
//        printf ("Size: %d\tMflop/s: %8g\tPercentage:%6.2lf\n", n, Mflops_s[isize],per[isize]);
      ...
      printf("%i x %i: Average percentage of Peak = %g\n",M_BLOCK_SIZE,N_BLOCK_SIZE,aveper);  
    }
      free (buf);
